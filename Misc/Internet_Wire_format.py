...
class Internet_Wire_Format:
    """
Construct array of bit fields representing an IETF RFC wire format for an
Internet protocol layer.

Notes about IETF internet protocol wire formats:
    1. they are always big-endian
    2. they always start at a given 2**n byte boundary
       * normally a 4-byte boundary for IPv4 and a 16-byte boundary for IPv6
       * this is specified in the declaration at a given memory boundary, normally 4 byte in IPv4, 16 byte in IPv6
    3. field names are specified in RFC, normally as upper case roman words or abbreviations
       * in this representation, spaces in RFC names are replaced with underscores
    4. for any Internet_Wire_Format instance iwf, str(iwf) returns ASCII art
    
"""
    
    def __init__(self,boundary=32):
    ...
    def boundary(self,bit_boundary:Nat)->Nat:
        ...
        current_bit_offset = self.bit_offset%bit_boundary
        #determine if already on display_width
