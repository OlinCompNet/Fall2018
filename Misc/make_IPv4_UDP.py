...
def decode_packet(sniffed = b'E\x00\x00\x1ea\xb8@\x00@\x11\xc4\xc6\n\n\x00 \n\n\x00\x1d\xb5\xc8\x13\x88\x00\n\xb9\xcfhie'):
    ...
    for g in UDP.field_name_list:
        ...
        dct["UDP Data"] = UDP_Data.decode("latin-1")
    return dct
...
#    udp_fields = ["UDP "+x.replace(' ', '_') for x in ]
    ...
    for i in (range(len(ipv4_fields)-1)):
   #     print(result)
        result = result << getattr(IPv4, ipv4_fields[i+1]).bit_length
  #      print(result)
        result = result | d['IPv4 '+ipv4_fields[i+1]]
 #       print(result)
#        print('\n')
